...
type CandidateTAG = {
  id: string;
  name: string;
  category: "application" | "external" | "system";
  order: number;
};
...
const upsertCandidate = async ({
  ...
  firstName,
  lastName,
}: Partial<Candidate>): Promise<Candidate> => {
  // find existing candidate by email
  const existingCandidate: Candidate | null = await fetch(
    INTERNAL_API_URL +
      "/candidates?query=" +
 ...
 */
const getListing = async (listing_id: string): Promise<Listing | null> => {
 ...
 */
export const submitApplication = async (input: ApplicationSubmission) => {
  ...
  let applicationTags: CandidateTAG[] = [];
  for (const i of tags) {
